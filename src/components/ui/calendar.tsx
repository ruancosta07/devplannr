  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        caption_label: "text-[1.8rem] font-medium first-letter:uppercase",
        nav: "space-x-1 flex items-center",
        nav_button: cn(
          buttonVariants({ variant: "outline" }),
          "h-10 w-10 nav-button bg-transparent p-0 opacity-50 hover:opacity-100"
        ),
        ...
        head_cell:
          "text-zinc-500 rounded-md w-9 font-normal text-[0.8rem] dark:text-zinc-400 w-full text-[1.2rem]",
        row: "flex w-full mt-2",
        cell: "h-16 w-16 text-center text-sm p-0 relative [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-zinc-100/50 [&:has([aria-selected])]:bg-zinc-100 first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20 dark:[&:has([aria-selected].day-outside)]:bg-zinc-800/50 dark:[&:has([aria-selected])]:bg-zinc-800",
        day: cn(
          buttonVariants({ variant: "ghost" }),
          "h-16 w-16 p-0 font-normal text-[1.2rem] aria-selected:opacity-100"
        ),
